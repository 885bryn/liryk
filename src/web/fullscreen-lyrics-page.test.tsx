...
describe("FullscreenLyricsPage", () => {
  ...
  it("prevents cumulative upward drift across 12 sustained transitions", async () => {
    ...
    nowPlayingResponse = {
      ...
      artist: "Viewport Artist",
      progressMs: transitionProgressPoints[0] ?? 0,
      isPlaying: true,
    };
  ...
  it("does not highlight active lyric before first synced timestamp", async () => {
    ...
    await waitFor(() => {
      ...
      // Track is pre-positioned to center line 0 even before it activates (no jump on first cue)
      const track = screen.getByTestId("fullscreen-lyrics-track");
      const translateY = parseFloat(track.style.transform.replace("translateY(", "").replace("px)", ""));
  ...
  describe("dev panel integration", () => {
    beforeEach(() => {
      // jsdom does not implement scrollIntoView — stub it so DevActivityPanel's
      // auto-scroll effect does not throw when the panel is opened in tests
    ...
    it("auth Connected event logged when status transitions to success", async () => {
      ...
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      // Simulate transition to connected (success)
      hookModel = {
        ...hookModel,
        uiState: {
          status: "success",
          successMessage: "Connected to Spotify",
          onboardingExplainer: disconnectedState.onboardingExplainer,
          permissionSummary: disconnectedState.permissionSummary,
        },
      };
    ...
    it("lyrics synced event logged", async () => {
      hookModel = {
        phase: "ready",
        statusCopy: "Connected",
        uiState: { status: "success", successMessage: "Connected", onboardingExplainer: disconnectedState.onboardingExplainer, permissionSummary: disconnectedState.permissionSummary },
        onConnect: async () => undefined,
        sessionAccessToken: "session-token",
      };
      nowPlayingResponse = { trackId: "track-lyrics-synced", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      resolvedLyricsResponse = { sourceState: "synced", renderMode: "synced", lines: [] };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[LYRICS\] Synced lyrics/)).toBeDefined(); });
    });

    it("lyrics plain event logged", async () => {
      hookModel = {
        phase: "ready",
        statusCopy: "Connected",
        uiState: { status: "success", successMessage: "Connected", onboardingExplainer: disconnectedState.onboardingExplainer, permissionSummary: disconnectedState.permissionSummary },
      ...
      nowPlayingResponse = { trackId: "track-lyrics-plain", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      resolvedLyricsResponse = { sourceState: "plain", renderMode: "plain-static", lines: [] };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[LYRICS\] Plain lyrics/)).toBeDefined(); });
    });

    it("lyrics low-confidence event logged", async () => {
      hookModel = {
        phase: "ready",
        statusCopy: "Connected",
        uiState: { status: "success", successMessage: "Connected", onboardingExplainer: disconnectedState.onboardingExplainer, permissionSummary: disconnectedState.permissionSummary },
      ...
      nowPlayingResponse = { trackId: "track-lyrics-lowconf", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      resolvedLyricsResponse = { sourceState: "low-confidence", renderMode: "plain-static", lines: [] };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[LYRICS\] Low-confidence lyrics/)).toBeDefined(); });
    });

    it("lyrics not-found event logged", async () => {
      hookModel = {
        phase: "ready",
        statusCopy: "Connected",
        uiState: { status: "success", successMessage: "Connected", onboardingExplainer: disconnectedState.onboardingExplainer, permissionSummary: disconnectedState.permissionSummary },
      ...
      nowPlayingResponse = { trackId: "track-lyrics-notfound", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      resolvedLyricsResponse = { sourceState: "not-found", renderMode: "plain-static", lines: [] };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[LYRICS\] No lyrics found/)).toBeDefined(); });
    });

    it("lyrics exception event logged", async () => {
      const { resolveLyricsForTrack } = await import("@/core/lyrics/lyrics-resolver");
      vi.mocked(resolveLyricsForTrack).mockRejectedValueOnce(new Error("fetch failed"));
      hookModel = {
        phase: "ready",
        statusCopy: "Connected",
        uiState: { status: "success", successMessage: "Connected", onboardingExplainer: disconnectedState.onboardingExplainer, permissionSummary: disconnectedState.permissionSummary },
      ...
      nowPlayingResponse = { trackId: "track-lyrics-exception", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      await act(async () => { rerender(<FullscreenLyricsPage />); });
    ...
    it("sync track changed event logged", async () => {
      nowPlayingResponse = { trackId: "track-sync-A", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      nowPlayingResponse = { trackId: "track-sync-B", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[SYNC\] Track changed/)).toBeDefined(); });
    });

    it("sync no active playback event logged", async () => {
      nowPlayingResponse = { trackId: "track-sync-null", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      nowPlayingResponse = null;
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[SYNC\] No active playback/)).toBeDefined(); });
    });

    it("sync playback resumed event logged", async () => {
      nowPlayingResponse = { trackId: "track-resume", title: "T", artist: "A", progressMs: 0, isPlaying: false };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      nowPlayingResponse = { trackId: "track-resume", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[SYNC\] Playback resumed/)).toBeDefined(); });
    });

    it("sync playback paused event logged", async () => {
      nowPlayingResponse = { trackId: "track-pause", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      nowPlayingResponse = { trackId: "track-pause", title: "T", artist: "A", progressMs: 0, isPlaying: false };
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[SYNC\] Playback paused/)).toBeDefined(); });
    });

    it("no spurious sync entry on mount", async () => {
      nowPlayingResponse = { trackId: "track-mount", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
    ...
    it("clock hard reset event logged on track change", async () => {
      nowPlayingResponse = { trackId: "track-clock-A", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      const { rerender } = render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
      nowPlayingResponse = { trackId: "track-clock-B", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      await act(async () => { rerender(<FullscreenLyricsPage />); });
      await waitFor(() => { expect(screen.getByText(/\[CLOCK\] Hard reset/)).toBeDefined(); });
    });

    it("no spurious clock entry on mount", async () => {
      nowPlayingResponse = { trackId: "track-clock-mount", title: "T", artist: "A", progressMs: 0, isPlaying: true };
      render(<FullscreenLyricsPage />);
      const toggle = screen.getByTestId("fullscreen-dev-panel-toggle");
      await act(async () => { fireEvent.click(toggle); });
