...
function FullscreenLyricsPageView({ embedded, webAuth }: FullscreenLyricsPageViewProps) {
  ...
  const preIntroOffsetPx =
    !canRenderSyncedMotion && resolvedLyrics?.renderMode === "synced" && syncedDisplayLines.length > 0
      ? -getFloatingRowAnchorPx(rowLayout, 0)
      : 0;
  ...
  useEffect(() => {
    ...
    if (currentTrackId !== prevClockTrackIdRef.current) {
      prevClockTrackIdRef.current = currentTrackId;
      if (currentTrackId !== null) {
        const rawDrift = syncState.driftDeltaMs;
        const absDrift = rawDrift < 0 ? -rawDrift : rawDrift;
        const driftMs = Math.round(absDrift);
