# Requirements Archive: v1.6 Developer Activity Panel
...
- [x] **DEV-01**: User can toggle the developer activity panel open/closed via a small button in fullscreen mode
- [x] **DEV-02**: Panel and toggle button render inside the fullscreen root element (not portaled to document.body) and are positioned absolutely so they do not affect lyric layout or fire the row ResizeObserver
- [x] **DEV-03**: Panel scroll events do not bubble to the lyric viewport scroll surface (live lock is not accidentally disengaged by scrolling the log)

### Event Log

- [x] **DEV-04**: Panel displays timestamped entries for lyrics fetch events (fetch initiated, cache hit, failure, provider used)
- [x] **DEV-05**: Panel displays timestamped entries for Spotify sync events (API poll, track change, playback state update)
- [x] **DEV-06**: Panel displays timestamped entries for playback clock events (drift corrections, hard resets)
- [x] **DEV-07**: Panel displays timestamped entries for auth/connection events (token refresh, auth state change, connection status)

### UX & Visual

- [x] **DEV-08**: Panel auto-scrolls to the latest entry, with a toggle to pause auto-scroll for inspection
- [x] **DEV-09**: Panel is styled to match the dark fullscreen aesthetic and does not disrupt lyric display
...
|-------------|-------|--------|
| DEV-01 | Phase 21 | Complete |
| DEV-02 | Phase 21 | Complete |
| DEV-03 | Phase 21 | Complete |
| DEV-04 | Phase 22 | Complete |
| DEV-05 | Phase 22 | Complete |
| DEV-06 | Phase 22 | Complete |
| DEV-07 | Phase 21 | Complete |
| DEV-08 | Phase 21 | Complete |
| DEV-09 | Phase 21 | Complete |
